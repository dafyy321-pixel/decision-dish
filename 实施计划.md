...
| 优先级 | 功能 | 状态 | 完成度 |
|--------|------|------|--------|
| **P0** | 核心功能（13项） | ✅ 全部完成 | **100%** |
| **P1** | 扩展功能（2项） | ✅ 历史记录已完成 ｜ ⏸️ 数据导入导出未开始 | **50%** |
| **P2** | 高级功能（2项：收藏功能、分享海报） | ⏸️ 未开始 | **0%** |
...
### ✅ 新增完成（P0+）
- 分享页面：系统分享（Web Share API）/ 复制链接 / 二维码生成（`src/pages/Share.tsx`）
- 底部导航栏：分享 / 收藏 / 抽奖 / 历史 / 我的（`src/components/BottomNavBar.tsx`）
- 反馈信息卡“必看！！”：问卷二维码 + 微信群二维码（`src/components/FeedbackCard.tsx`）
- Supabase 数据接入：`src/lib/supabase.ts` + `src/hooks/useRestaurants.ts`，类型 `src/types/database.types.ts`
- 分享页顶部标题图：`public/imeng-2025-11-02-8389-变成黑色文字，白色背景.png`

### ✅ 新增完成（P1）
#### 历史记录功能 ✅
**PRD 要求**: 查看最近抽取的餐厅（最近 10 次）

**实际实现**（在 `src/pages/Index.tsx` 中）：
- 每次抽取后自动保存记录到 localStorage
- Storage Key: `"draw_history"`
- 数据结构:
```typescript
interface HistoryRecord {
  name: string;         // 餐厅名称
  timestamp: string;    // 时间戳 (中文格式)
  mode: "system" | "custom";  // 抽取模式
}
```
- 最多保存 100 条记录（超出 PRD 要求）
- 同时统计总抽取次数 (key: `total_draws`)
- 记录最后抽取时间 (key: `last_draw_time`)

**实现位置**: `src/pages/Index.tsx:94-119` (在 `handleWheelComplete` 函数中)

> 注：历史记录展示 UI 页面（`/history`）已实现，支持查看/收藏/删除/清空
...
#### 2. 历史记录 UI 展示页 ✅（已上线，支持查看/收藏/删除/清空）
...
│   ├── SpinWheel.tsx          ✅ 转盘动画
│   ├── FeedbackCard.tsx       ✅ 反馈信息卡
│   ├── BottomNavBar.tsx       ✅ 底部导航栏
├── data/
│   └── restaurants.ts         ✅ 餐厅数据（20条）
├── hooks/
│   ├── useRestaurants.ts      ✅ Supabase 数据获取
│   └── useFavorites.ts        ⏸️ 收藏数据（预留）
├── lib/
...
│   ├── History.tsx            ✅ 历史页（列表/收藏/删除/清空）
│   ├── Profile.tsx            ✅ 我的（统计 + Top5）
│   └── NotFound.tsx           ✅ 404页面
├── types/
...
1. 数据导入导出（预计 3-5 天）
   ...
   - 数据校验/去重/限制 20 项
3. GA4 埋点（预计 1-2 天）
   - 接入核心事件：mode_selected/draw_clicked/draw_result/draw_again

**第二阶段 - P2 功能**:
3. 收藏功能（预计 5-7 天）
   - 结合 Supabase `user_favorites` 表
   - 结果页一键收藏 + 收藏列表抽取

4. 分享海报（预计 7-10 天，需要设计资源）

### 方案 B: 快速迭代（适合快速验证需求）
1. ✅ 历史记录存储（已完成） → 开发 UI 页面（最小化）
2. 数据导出（单向功能）
3. 收藏功能（最小化实现）
4. 根据用户反馈决定是否开发分享海报
...
| --- | --- | --- | --- | --- |
| v1.1 | 2025-11-03 ~ 2025-11-16 | 可记录与迁移数据 | ✅ 历史记录存储、❌ 历史UI、❌ 数据导入导出 | ✅ 最多100条历史存储；❌ 历史页UI；❌ JSON导入导出 |
| v1.2 | 2025-11-17 ~ 2025-11-30 | 提升分享与沉淀 | 收藏功能、分享海报 | 收藏增删查、从收藏抽取；生成海报并保存/复制；移动端清晰度达标（≥2x） |
| v1.3 | 2025-12-01 ~ 2025-12-14 | 体验与增长 | 桌面适配、PWA、埋点完善 | Lighthouse PWA ≥ 80；快捷键触发抽取；GA4 核心事件全量上报 |
