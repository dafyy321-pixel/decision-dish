...
| 优先级 | 功能模块 | 功能描述 | 实现状态 |
| --- | --- | --- | --- |
| P0 | 闪屏页面 | 1.5s 品牌展示页，渐隐效果 | ✅ 已实现 |
| P0 | 模式切换 | 系统推荐/自定义模式切换 | ✅ 已实现 |
| P0 | 系统推荐 | 从云端 Supabase 或 20 家预设餐厅随机抽取（自动回退） | ✅ 已实现 |
| P0 | 自定义列表 | 添加/删除/管理自定义餐厅（最多20个） | ✅ 已实现 |
| P0 | 转盘动画 | 1.3s 旋转动画 + 渐隐效果 | ✅ 已实现 |
| P0 | 结果展示 | 展示餐厅名称/地址/分类 | ✅ 已实现 |
| P0 | 再抽一次 | 重置结果，重新抽取 | ✅ 已实现 |
| P0 | 本地存储 | localStorage 持久化自定义数据 | ✅ 已实现 |
| P0 | Toast 提示 | 操作反馈（成功/错误提示） | ✅ 已实现 |
| P1 | 数据导入导出 | JSON 格式导入导出自定义列表 | ⏸️ 待开发 |
| P1 | 历史记录 | 查看最近抽取的餐厅 | ⏸️ 待开发 |
| P2 | 分享功能 | 生成图片分享到社交媒体 | ⏸️ 待开发 |
| P2 | 收藏功能 | 收藏喜欢的餐厅 | ⏸️ 待开发 |

#### 新增已实现功能（同步进度）

| 优先级 | 功能模块 | 功能描述 | 实现状态 |
| --- | --- | --- | --- |
| P0 | 底部导航栏 | 分享/收藏/抽奖/历史/我的 | ✅ 已实现 |
| P0 | 分享页面 | 系统分享/复制链接/二维码分享 | ✅ 已实现 |
| P0 | 反馈卡片 | “必看！！”问卷与微信群二维码 | ✅ 已实现 |
| P0 | Supabase 集成 | 云端餐厅数据（restaurants）获取与缓存 | ✅ 已实现 |

### 2.2 功能详细规格

#### 1️⃣ 闪屏页面 (Splash Screen)
...
- 优先从 Supabase public.restaurants 表获取 is_active=true 的餐厅列表，按名称排序。
- 若请求失败或无数据，自动回退到本地 src/data/restaurants.ts 预设列表。
...
| ------- | --- | ---------- | -------------------- | --- |
| 💥 闪屏页  | - | 品牌展示，1.5s 自动跳转 | 全屏 Logo图片、渐隐动画 | 全屏适配 |
| 🏠 主页面  | `/` | 核心功能页 | Logo+模式切换+抽取区+结果区 | 最大宽度 448px，居中 |
| 🚫 404页 | `*` | 错误页面 | 错误提示 + 返回数据按钮 | 居中显示 |
...
| `splash_view` | 闪屏页显示 | `duration` | 统计闪屏时长 |
...
- 历史记录（最近 10 次）
- 埋点：接入 GA4，完成核心事件上报
- 体验：动画时间个性化设置（1.0s/1.3s/2.0s）

### v1.2（两周）
- 邀请分享：生成分享海报（结果截图 + 文案）
- 收藏常用：结果页一键收藏、系统/自定义融合抽取模式
- 适配：桌面端细节优化（快捷键触发抽取）
...
├── public/                    # 静态资源
│   ├── favicon.png            # 网站图标
│   └── splash-screen.png      # 闪屏图片
├── src/
│   ├── assets/               # 图片资源
│   │   ├── title-logo.png    # 标题 Logo
│   │   └── splash-screen.png # 闪屏图
│   ├── components/           # React 组件
│   │   ├── ui/              # shadcn/ui 基础组件
...
│   │   └── useFavorites.ts       # 收藏数据（预留）
│   ├── lib/                 # 工具库
│   │   └── supabase.ts           # Supabase 客户端
│   ├── pages/                # 页面组件
│   │   ├── Index.tsx       # 主页 (SPA 核心页)
│   │   ├── Share.tsx       # 分享页（系统分享/复制链接/二维码）
